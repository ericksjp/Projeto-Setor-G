...
# driver.get('https://www.gtabase.com/gta-san-andreas/vehicles/#sort=attr.ct1.frontend_value&sortdir=asc&attr.ct1.value=boats&page=1')
...
url = 'https://www.gtabase.com/gta-san-andreas/vehicles/#sort=attr.ct1.frontend_value&sortdir=asc&attr.ct1.value=boats&page=1'
...
class_names = ['row', 'products', 'list', 'items', 'product-items', 'clearfix', 'cols-4']


titulos=[]
for i, td in enumerate(soup.find_all ('article')):
    print(td)
    ja-products-wrapper products wrapper cols-4 grid products-grid
    # title = td.get('title')
